...
# ── Config ──────────────────────────────────────────────────────────────────
BASE_OUTPUT_DIR = Path("/n/netscratch/kempner_undergrads/Everyone/zwu/parallel_toy/figures/laion_5k_generated_pt")
...
for tsr_k in K_VALUES:
	...
	for idx, prompt in enumerate(tqdm(prompts, desc=f"k={tsr_k}")):
		...
		try:
			...
			# With REPLICA_EXCHANGE=False there's only 1 image, but loop is safe either way
