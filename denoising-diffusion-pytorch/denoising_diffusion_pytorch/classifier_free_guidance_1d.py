...
class Unet1D(Module):
    ...
    def forward_with_cond_scale(
        ...
        **kwargs
        ...
        std_fn = partial(torch.std, dim = tuple(range(1, scaled_logits.ndim)), keepdim = True, unbiased = False)
        logits_std = std_fn(logits)
        scaled_logits_std = std_fn(scaled_logits)
        
        # Add small epsilon to prevent division by zero and handle edge cases
        eps = 1e-8
        # If scaled_logits_std is too small, skip rescaling to avoid NaNs
        scale_factor = torch.where(
            scaled_logits_std > eps,
            logits_std / (scaled_logits_std + eps),
            torch.ones_like(logits_std)
        )
        rescaled_logits = scaled_logits * scale_factor
...
class GaussianDiffusion1D(Module):
    ...
    def model_predictions(self, x, t, k, k_cns, classes, cond_scale = 6., rescaled_phi = 0.7, clip_x_start = False):
    ...
    def p_mean_variance(self, x, t, k, k_cns,classes, cond_scale, rescaled_phi, clip_denoised = True):
        preds = self.model_predictions(x, t,  k, k_cns, classes, cond_scale, rescaled_phi)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, k, k_cns, classes, cond_scale = 6., rescaled_phi = 0.7, clip_denoised = True):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((x.shape[0],), t, device = x.device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x = x, t = batched_times, k= k, k_cns=k_cns, classes = classes, cond_scale = cond_scale, rescaled_phi = rescaled_phi, clip_denoised = clip_denoised)
        noise = torch.randn_like(x) if t > 0 else 0. # no noise if t == 0
        
        noise = noise * (1/(math.sqrt(k_cns))) # constant noise rescaling

        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
        return pred_img, x_start

    @torch.no_grad()
    def p_sample_loop(self, k, k_cns, classes, shape, cond_scale = 6., rescaled_phi = 0.7):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            img, x_start = self.p_sample(img, t, k, k_cns, classes, cond_scale, rescaled_phi)

        img = self.unnormalize(img)
        return img

    @torch.no_grad()
    def ddim_sample(self, k, k_cns,classes, shape, cond_scale = 6., rescaled_phi = 0.7, clip_denoised = True):
    ...
    @torch.no_grad()
    def sample(self, k, k_cns, classes, cond_scale = 6., rescaled_phi = 0.7):
        ...
        return sample_fn(k, k_cns, classes, shape, cond_scale, rescaled_phi)
...
if __name__ == '__main__':
    ...
    training_data = torch.randn(8, 3, 128).cuda() # 1D data is normalized from 0 to 1
    data_classes = torch.randint(0, num_classes, (8,)).cuda()    # say 10 classes

    loss = diffusion(training_data, classes = data_classes)
    loss.backward()

    # do above for many steps

    sampled_data = diffusion.sample(
