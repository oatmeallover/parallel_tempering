	...
	def __call__(
		...
		with self.progress_bar(total=num_inference_steps) as progress_bar:
			for i, t in enumerate(timesteps):
				...
				if replica_exchange:
					# Each replica gets the same text conditioning — tile across replicas
					prompt_embeds_inputs = prompt_embeds.repeat_interleave(n_replicas, dim=0)         # [2*n_replicas, seq, dim]
					pooled_prompt_embeds_inputs = pooled_prompt_embeds.repeat_interleave(n_replicas, dim=0)  # [2*n_replicas, dim]
				else:
					prompt_embeds_inputs = prompt_embeds
					pooled_prompt_embeds_inputs = pooled_prompt_embeds

				noise_pred = self.transformer(
					hidden_states=latent_model_input,
					timestep=timestep,
					encoder_hidden_states=prompt_embeds_inputs,
					pooled_projections=pooled_prompt_embeds_inputs,
				...
				if tsr_lam is not None and tsr_sigma is not None: # NEW CODE
					...
					if replica_exchange:
						...
						tsr = compute_tsr_constant(t, tsr_lam, sigma_i, tsr_sigm, replica_exchange=False)
						noise_pred *= tsr
				...
				if replica_exchange: # NEW CODE
					sigma_i = self.scheduler.sigmas[i].to(device=latents.device, dtype=latents.dtype)
					alpha_bars = (1.0 - self.scheduler.sigmas[:-1].to(device=device, dtype=latents.dtype)) ** 2
					alpha_bar_i = alpha_bars[i]
					latents = exchanged_replicas(
		...
		if output_type == "latent":
			...
			# Decode each replica separately to avoid OOM
			decoded = []
			for i in range(latents.shape[0]):
				latent_i = latents[i:i+1]  # [1, C, H, W]
				decoded.append(self.vae.decode(latent_i, return_dict=False)[0])
